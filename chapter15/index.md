...
Sofern nicht anders gekennzeichnet stammen alle folgenden Informationen und Abbildungen aus dem Buch
"Software Engineering (10th ed.)" von Ian Sommerville [1].
...
1. **System-Wiederverwendung** Komplette Systeme, welche aus einer Anzahl an Applikationsprogrammen bestehen, können als Teil eines Systems bestehend aus Systemen wiederverwendet werden.
...
und kann so für jedes neue Problem angepasst werden. Die Konzept-Wiederverwendung wird in Design Pattern, konfigurierbaren 
...
| Vorteil          |     Erklärung       |
|:----------:|:----------------:|
| Schnellere Entwicklung | Ein System so schnell wie möglich auf den Markt zu bringen ist teils wichtiger als die Entwicklungskosten. Wiederverwendung von Software kann die Entwicklungszeit beschleunigen. |
| Effektiver Einsatz von Spezialisten  | Statt die gleiche Arbeit immer wieder zu machen, können Spezialisten wiederverwendbare Software entwickeln, welche ihr Wissen einfängt. | 
| Erhöhte Zuverlässigkeit | Wiederverwendete Software, welche bereits in funktionierenden Systemen getestet wurde, sollte zuferlässiger als neue Systeme sein. Die Design- und Implementierungsfehler sollten bereits gefunden sein. |
| Niedrigere Entwicklungskosten | Entwicklungskosten sind proportional zu der grösse des Softwaresystems. Software wiederzuverwenden bedeutet, dass weniger Programmcode geschrieben werden muss. |
| Reduziertes Prozessrisiko | Die Kosten von existierender Software sind bereits bekannt, während die Kosten der Entwicklung immer geschätzt sind. Das Wiederverwenden kann so den Fehler in der Projektkosten-Schätzung verringern. Dies ist gerade dann wahr, wenn grosse Softwarekomponenten wie Subsysteme wiederverwendet werden können. |
| Einhaltung von Standards | Manche Standards können als Sammlung von wiederverwendbarer Software implementiert werden. Die Verwendung von Standardbenutzeroberflächen zum Beispiel verbessert die Zuverlässlichkeit, weil Benutzer weniger Fehler machen, wenn sie mit vertrauten Oberflächen konfrontiert werden. |

Folgend werden Nachteile tabellarisch dargestellt.

| Nachteil          |     Erklärung       |
|:----------:|:----------------:|
| Erstellen, Pflegen und Verwenden von Komponentenbibliotheken | Das Befüllen einer wiederverwendbaren Komponentenbibliothek und das Sicherstellen, dass die Software vom Entwickler verwendet werden kann, ist teuer. Entwicklungprozesse müssen angepasst werden, um sicherzustellen, dass die Bibliothek verwendet wird.  |
| Finden, Verstehen und Anpassung wiederverwendbarer Komponenten  | Softwarekomponenten müssen in Bibliotheken gefunden, verstanden und manchmal an die neue Umgebung angepasst werden. Ingenieure müssen halbwegs zuversichtlich sein, Komponenten in einer Bibliothek zu finden, bevor sie eine Komponentensuche als Teil ihrer Normalität in den Entwicklungsprozess einschließen. | 
| Erhöhte Wartungskosten | Wenn der Quellcode eines wiederverwendbaren Systems oder einer Komponente nicht verfügbar ist, dann können Wartungsarbeiten wegen der Wiederverwendung höher sein, weil die wiederverwendbaren Teile des Systems eventuell nach Veränderung des Systems inkompatibel werden. |
| Fehlende Werkzeugunterstützung | Manche Softwaretools unterstützen keine Entwicklung mit Wiederverwendung. Es kann so unmöglich sein, diese Tools mit einem Bibliothekssystem zu integrieren. Dies ist bei Tools, welche Embedded-System-Entwicklung unterstuetzen, eher der Fall als bei objektorientierten Entwicklungstools. |
| "Nicht-hier-erfunden"-Syndrom | Manche Entwickler bevorzugen es Komponenten selber zu entwickeln, da sie der Meinung sind, diese verbessern zu können. Dies liegt unter anderem an Vertrauen und teils daran, dass es als herausfordernder angesehen wird, als die Software von anderen Personen wiederzuverwenden. |
...
|Architekturmuster | Standardsoftwarearchitekturen, die gängige Arten von Anwendungssystem unterstützen, dienen als Basis von Anwendungen. |
|Aspektorientierte Software-Entwicklung | Gemeinsame Komponenten werden an verschiedenen Stellen in eine Anwendung eingewoben, wenn das Programm kompiliert wird. |
|Komponentenbasierte Softwareentwicklung | Systeme werden entwickelt, indem Komponenten integriert werden (Sammlungen von Objekten), die den Komponentenmodellstandards entsprechen. |
...
|Design Patterns | Generische Abstraktionen, die anwendungsübergreifend auftreten, werden als Entwurfsmuster dargestellt, welche abstrakte und konkrete Objekte und Interaktionen zeigen. |
...
|Legacy-Systemumhüllung | Legacy-Systeme werden „umhüllt“, indem eine Menge von Interfaces definiert werden. So wird der Zugriff auf diese Legacy-Systeme über beschriebene Interfaces bereitgestellt. |
|Modellgetrieben Entwicklung | Software wird als Domänenmodelle und als implementierungsunabhängige Modelle dargestellt. Aus diesen Modellen wird Code generiert. |
...
|Serviceorientierte Systeme | Systeme werden durch die Verknüpfung gemeinsam genutzter Dienste entwickelt, die ggf. extern bereitgestellt werden. |
...
|Systeme von Systemen | Zwei oder mehr verteilte Systeme werden integriert, um ein neues System zu erstellen. |

Im Folgenden werden die Hauptfaktoren beschrieben, welche beachtet werden sollten,
wenn die Wiederverwendbarkeit von Software geplant ist.
...
4. **Kritikalität der Software und ihre nicht funktionalen Anforderungen** Für ein kritisches System, welches durch einen externen Regulator zertifiziert werden muss, müssen eventuell Sicherheitsfälle für das System aufgestellt werden. Dies ist schwierig, falls der Quellcode nicht komplett einsehbar ist.
...
wiederverwendbaren Teilen und Prozessen fürs Erleichtern der Wiederverwendung konzentriert [2].
...
Applikation angepasst sind. Meist dauert es länger, vorhandene Objekte zu verstehen und anzupassen, als diese neu zu implementieren.
...
Schmidt et al. [3] definiert ein Framework als 

"ein integrierten Satz von Softwareartefakten (wie Klassen, Objekte und Komponenten), die
...
verwendet und basieren dabei meist auf dem "Model-View-Controller (MVC) Pattern". Das Pattern wurde 1980 als Ansatz
...
Frameworks verwenden sehr oft Design Pattern. MVC-Frameworks verwenden beispielsweise das Observer-Pattern, das 
...
2. **Dynamische Webseiten** Klassen helfen dabei Webseitenvorlagen zu definieren und diese dynamisch mit Daten aus der Datanbank zu füllen.
3. **Datenbankintegration** Frameworks stellen eventuell Klassen zur Verfügung, welche eine abstrakte Schnittstelle zu verschiedenen Datenbanken bereitstellen.
4. **Sitzungsverwaltung** Klassen, welche Sitzungen erstellen und verwalten, sind oft Teil eines WAF.
5. **Benutzerinteraktionen** Web Frameworks liefern Supprt für AJAX [4] und/oder HTML5 [5] zur Erstellung interaktiver Webseiten. Darunter können sich Klassen zur Erstellung geräteunabhängiger Interfaces befinden, welche sich automatisch an Mobiltelefone und Tablets anpassen.
...
Event vom Framework erkannt wird. Die Frameworkobjekte sind dabei für die Kontrolle zuständig, nicht die applikationsspezifischen
Objekte. Dies wird "Inversion of Control" [3] genannt.
...
Fayad und Schmidt [6] diskutieren drei weitere Klassen von Frameworks:
...
2. **Frameworks für die Middleware-Integration** bestehen aus einer Menge an Standards und damit verbundenen Objektklassen, welche die Kommunikation und den Datenaustausch der Komponenten unterstützt. Beispiele sind Microsoft's .NET und Enterprise Java Beans (EJB).
3. **Frameworks für Unternehmensanwendungen** behandeln konkrete Anwendungensgebiete wie Telekommunikations- und Finanzsysteme [7]. Diese beinhalten Wissen über Andwendungsbereiche und unterstützen die Entwicklung von Endbenutzeranwendungen. Heute sind sie nicht verbreitet und wurden weitgehend durch Software-Produktlinien ersetzt.
...
geeignetsten ist. Zusätzlich kann das Debuggen mit Frameworks schwierig sein, da man nicht zwingend weiss, wie die Frameworkmethoden
...
konfiguriert und so an Kundenwünsche oder bestimmtes Equipment angepasst werden. Dazu können Komponenten konfiguriert, zusätzliche
...
vereinfacht werden. Tests für grosse Teile der Applikation können eventuell wiederverwendet werden. So wird die Entwicklungszeit
...
für eine allgemeine Ressourcenzuweisungs- und Management-Architektur. Im Folgenden wird eine solche Architektur dargestellt.
...
Ressourcen an, geben Ressourcen frei und können Fragen bezüglich Ressourcen und deren Verfügbarkeit stellen.
...
besitzt viele, jedoch wenig verschiedene Autos. Die Feuerwehr hingegen besitzt wenige, dafür aber sehr verschiedene und
...
2. **Umgebungs-Spezialisierung** Versionen der Anwendung können in verschiedenen Betriebsumgebungen mit verschiedenen Peripheriegeräten benötigt werden. Beispielsweise kann ein System für Rettungsdienste in verschiedenen Versionen, abhängig von benutzter Hardware, exisitieren. Polizeistationen kommunizieren eventuell mit eingebaueter Verschlüsselung. Die Produktlinien-Komponenten werden der Funktionalität der verwendeten Ausrüstung angepasst.
3. **Funktionelle Spezialisierung** Versionen der Anwendung sind für verschiedene Kunden mit verschiedenen Anforderungen. Beispielsweise muss ein Bibliothek-Automatisierungs-System angepasst werden, wenn es sich um eine öffentliche Bibliothek oder eine Universitätsbibliothek handelt. Komponenten mit Funktionalität müssen eventuell überarbeitet oder neu geschrieben werden.
...
2. **Auswählen eines vorhandenen Systems, das den Anforderungen am besten entspricht** Die Anforderungen werden analysiert und ein Produktlinien-Mitglied, welches am nächsten an die neuen Anforderungen herankommt, zur Modifikation ausgewählt.
...
4. **Bestehendes System anpassen** Neue Module für das existierende System werden entwickelt und vorhandene Module werden den Anforderungen entsprechend angepasst.
...
Wenn ein System zur Entwurfszeit konfiguriert wird, startet der Hersteller mit einem generischen oder einem vorhandenen System.
...
"Commercial Off-the Shelf Systeme" (COTS) genannt. Da der Begriff meist in Militärsystemen verwendet wird, werden sie im Folgenden
weiterhin Applikationsysteme genannt.
...
Umgebungen und als Teil verschiedener Systeme wiederverwendet werden. Torchiano and Morisio [8] fanden
...
3. Manche Entwicklungsrisiken sind beim Verwenden vorhandener Software minimiert.
...
5. Wenn sich Operationsplatformen verändern, ist das aktualisieren vereinfacht, da die Verantwortung dafür bei dem Applikationssystem-Anbieter liegt.
...
3. Das passende Applikationssystem auszusuchen kann ein schwieriger Prozess sein, da diese oft nicht gut dokumentiert sind. Bei falschen Entscheidungen kann es unmöglich sein, das neue System nach den Anforderungen umzusetzen.
...
Konfigurierbare Applikationssysteme sind generische Applikationssysteme, welche für bestimmte Unternehmenstypen,
...
ein Enterprise Resource Planning (ERP) System die Herstellung, den Einkauf und das Kundenbeziehungsmanagement in grossen 
...
Universität einschreiben können. Bei einer Kollaberation zweier Universitäten werden gemeinsame Abschlüsse angeboten, und
...
Rechnungsstellung, Bestandsverwaltung und Fertigungsplanung [9]. Im Konfigurationsprozess dieser Systeme müssen
...
wird der Prozess der Konfigurierung von Beratern, die mit dem Systemkunden zusammenarbeiten, ausgeführt.
...
benutzte Form der Wiederverwendung dar. Eine Limitierung ist es, dass die Funktionalität des Systems des Kundens an die
...
Erstellung können Nichtübereinstimmungen zwischen den Annahmen des Systemanbieters und dem Kunden bezüglich
...
ERP Systeme benötigen grundsätzlich eine umfangreiche Konfiguration, um an die Anforderungen des Kunden angepasst zu werden. Diese Konfiguration enthält:
...
3. Definition von Geschäftsregeln, die für diese Daten gelten.
...
anderen Systems verknüpft oder die Datenbank, welche von den Applikationen verwendet wird, angepasst werden.
...
Das Integieren von Applikationssystemen ist das gleiche wie das Integrieren jeder anderer Komponente auch. Es muss die 
...
und Abts [10] heben vier wichtige Systemintegrationsprobleme hervor:

1. **Mangelnde Kontrolle über Funktionalität und Leistung** Auch wenn eine Schnittstelle eines Produkts vielversprechend aussieht, weiss man nicht, ob das System nicht vielleicht schlecht implementiert ist, oder schlecht abschneidet. Das Produkt hat eventuell versteckte Operationen, welche bei der Benutzung in bestimmten Situationen stören können. Diese Probleme zu beheben kann sehr wichtig für den Systemintegrator sein, jedoch von weniger Bedeutung für den Systemanbieter selbst. Benutzer müssen so eventuell Problemumgehungen finden, wenn das System wiederverwendet werden soll.
2. **Probleme mit der Systeminteroperabilität** Es ist schwierig individuelle Systeme miteinander arbeiten zu lassen, da in beiden Systemen Annahmen getroffen wurden, wie diese verwendet werden. Garlan et al. [11], berichtet von einer Integration von vier Systemen. Drei Systeme waren eventbasiert, benutzten jedoch verschiedene Modelle von Events. Die Integration dauerte zwei Jahre statt die geschätzten sechs Monate. Torchiano and Morisio [8] fanden heraus, dass das Nichteinhalten von Standards die Integrierung erschwert.
3. **Keine Kontrolle über die Systementwicklung** Systemanbieter können alleine über Änderungen des Systems entscheiden. Bei PC Produkten heisst dies, dass oft neue Versionen veröffentlicht werden, welche eventuell nicht mit alten Versionen kompatibel sind. Neue Versionen haben eventuell neue ungewollte Funktionalitäten und alte Versionen können nicht mehr verfügbar und unterstützt werden.
4. **Unterstützung durch Systemanbieter** Anbieter bieten ganz unterschiedlich viel Support für ihre Produkte. Wenn Probleme auftreten ist die Unterstützung durch den Systemanbieter notwendig, da Kunden keinen Zugriff auf Programmcode oder detaillierte Dokumentation haben. Der sich verändernde Markt und ökonomische Umstände können dazu führen, dass der Systemanbieter nicht die Unterstützung liefern kann, die der Kunde benötigt. Ein Produkt kann zum Beispiel aus dem Sortiment genommen werden, weil es nur eine begrenzte Nachfrage generiert.

Böhm and Abts [10] vermuten, dass die Kosten für die Systemwartung und -weiterentwicklung bei integrierten Applikationssystemen grösser sind. Je weiter die Leute, welche für die Systemwartung zuständig sind, von den eigentlichen Entwicklern des Systems getrennt sind, desto mehr Probleme mit dem integrierten System können auftreten.
