...
Wenn ein System als zuverlässig eingestuft werden kann, heißt das nicht automatisch das dies ebenfalls sicher ist. Es gibt vier verschiedene Gründe warum ein zuverlässiges Softwaresystem nicht sicher sein kann:
...
Unter einem Sicherheitskritischen System ist zu verstehen, dass das System unter keinen Umständen einem Menschen oder der Umwelt schädigen darf und das unabhängig davon, ob sie sich wie in der Spezifikation definiert verhält oder dies nicht tut. Es gibt zwei unterschiedliche Typen von sicherheitskritischer Software:

1. **Primär sicherheitskritische Software**:
   * Hierbei handelt es sich um eine Software, wo ein Fehler dazu führen kann, dass die Hardware nicht mehr ordnungsgemäß funktioniert und somit ein Mensch oder die Umwelt verletzt werden kann.
2. **Sekundär sicherheitskritische Software**:
   * Bei dieser Art von Software führt ein Fehlverhalten nicht direkt zu einem Problem, sondern es besteht die Möglichkeit, dass nachfolgende Operationen aufgrund dessen zu einem Fehler führen.

Wichtig hierbei ist die Unterscheidung zwischen einer Gefahr und einen Unfall. Eine Gefahr ist ein Zustand eines Systems, wodurch ein Unfall entstehen kann. Somit sind diese beiden Begriffe nicht gleichzusetzen. Das heißt jedoch, dass die Reduzierung einer Gefahr ebenfalls dazu führt, dass die Anzahl an Unfälle reduziert werden. In der folgenden Tabelle sind die verschiedenen Definitionen der einzelnen Begriffe aufgeführt.
...
Gefahrenwahrscheinlichkeit | Die Wahrscheinlichkeit des Eintretens von Ereignissen, die eine Gefährdung erzeugen. Wahrscheinlichkeitswerte sind in der Regel zufällig, reichen aber von „wahrscheinlich“ (z. B. 1/100 Wahrscheinlichkeit, dass eine Gefahr eintritt) bis „unglaubwürdig“ (es sind keine denkbaren Situationen wahrscheinlich, in denen die Gefahr auftreten könnte).
...
2. **So gering wie vernünftigerweise praktikabel – ALARP**
   * Dabei handelt es sich um zwei mögliche Risiken:
...
In jeder Phase beim Entwickeln einer Software muss gewährleistet werden, dass das System sicher ist. Dafür gibt es eine Reihe von Aktivitäten die das überprüfen. Zusätzlich zu den Tätigkeiten muss die dafür verantwortliche Person dokumentiert werden. Die Dokumentation der zuständigen Person ist wichtig, da zum einen diese verantwortungsvoller mit der Aufgabe umgeht und zum anderen im Falle von einer rechtlichen Auseinandersetzung, bei einem Unfall, diese sich verteidigen kann. Bei dem Entwickeln von sicherheitskritischen Systemen ist zu beachten, dass häufiger externe Komponenten verwendet werden und diese unter Umständen mit anderen Standards umgesetzt wurden. Daher müssen diese ebenfalls kontrolliert und zertifiziert werden. Somit ist die Prozesssicherung ein wichtiger Bestandteil bei der Entwicklung dieser Systeme. Mit alleinigen Testen können unter Umstände nicht alle Probleme gefunden werden, die in seltenen Fällen zu einen Unfall führen. Das liegt an den "soll nicht"-Anforderungen, da diese nur schwer mit einem Test zu überprüfen sind. Aus diesem Grund werden Aktivitäten zum Gewährleisten der Sicherheit durchgeführt. Darunter zählen:
...
Fehlerklasse              | Überprüfung der statischen Analyse
--------------------------|-----------------------------------
Datenfehler               | Vor der Initialisierung verwendete Variablen / Deklarierte, aber nie verwendete Variablen / Zweimal zugewiesene Variablen, die zwischen den Zuweisungen nie verwendet werden / Mögliche Verletzungen der Array-Begrenzung / Nicht deklarierte Variablen
Steuerungsfehler          | Unerreichbarer Code / Unbedingte Verzweigungen in Schleifen
Ein-/Ausgangsfehler       | Variablen werden zweimal ohne dazwischenliegende Zuweisung ausgegeben
Schnittstellenfehler      | Nicht übereinstimmende Parametertypen / Nicht übereinstimmende Parameternummern / Keine Nutzung der Ergebnisse von Funktionen, nicht aufgerufene Funktionen und Prozeduren
Speicherverwaltungsfehler | Nicht zugewiesene Zeiger / Speicherlecks
...
Für die Überprüfung von sicherheitskritischen Systemen gibt es bestimmte Regularien, die von externen Regulierungsbehörden überprüft werden können. Um zu beweisen, dass das eigene System nicht schädlich ist, wird ein Sicherheitsnachweis erstellt. Dieser Nachweis enthält verschiedene Informationen. Eine Beispielhafte Auflistung der unterschiedlichen Kapitel ist in der nachfolgenden Tabelle abgebildet:
